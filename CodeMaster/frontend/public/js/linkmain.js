document.addEventListener("DOMContentLoaded", function () {
	...
	window.addEventListener("resize", () => {
		document.querySelector("main").style.height =
			window.innerHeight - document.querySelector("header").scrollHeight + "px";
	});
	...
	function getUserInput(parentNode) {
		...
		if (inputs.length === 1) {
			...
			else userInput.index = inputs[0].valueAsNumber;
			return userInput;
		}
	...
	removeBtn.addEventListener("click", () => {
		let input = null;
		for (let i of inputs)
			if (i.style.display !== "" && i.style.display !== "none") input = i;
		...
		if (input.placeholder === "Index") removeIndex(input.valueAsNumber);
		else removeData(input.valueAsNumber);
	});
	...
	document
		.getElementById("remove-settings")
		.addEventListener("click", function () {
			...
			if (!displayed) {
				this.firstChild.nextSibling.style.animation =
					"removeAnimationOn " + menuAnimationTimeout / 1000 + "s " + "ease";

				removeIndexBtn.style.display = "block";
				removeIndexBtn.style.animation =
					"toggleMenuDown " + menuAnimationTimeout / 1000 + "s " + "ease";

				removeDataBtn.style.display = "block";
				removeDataBtn.style.animation =
					"toggleMenuDown200 " + menuAnimationTimeout / 1000 + "s " + "ease";

				inputs[0].style.display = "none";
				inputs[1].style.display = "none";
			} else {
				this.firstChild.nextSibling.style.animation =
					"removeAnimationOff " + menuAnimationTimeout / 1000 + "s " + "ease";

				removeIndexBtn.style.animation =
					"toggleMenuUp " + menuAnimationTimeout / 1000 + "s " + "ease";

				removeDataBtn.style.animation =
					"toggleMenuUp200 " + menuAnimationTimeout / 1000 + "s " + "ease";
