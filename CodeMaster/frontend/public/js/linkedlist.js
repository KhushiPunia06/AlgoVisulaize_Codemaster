...
function deleteNode(i) {
	return new Promise((resolve) => {
		nodes[i].style.animation = "deleteNode " + deleteTimeout / 1000 + "s ease";
		pointers[i].style.animation =
...
function animateNodesAfterRemove(from) {
	return new Promise((resolve) => {
		for (let i = from; i < nodes.length; i++) {
			nodes[i].style.animation =
				"moveLeftNode " + deleteTimeout / 1000 + "s " + "ease";
...
async function add(i, data) {
	if (checkInputErrors(i, "Index") || checkInputErrors(data, "Data")) return;
...
async function removeIndex(i) {
	if (handleEmptyListError() || checkInputErrors(i, "Index", true)) return;
...
function removeData(data) {
	if (handleEmptyListError() || checkInputErrors(data, "Data")) return;
