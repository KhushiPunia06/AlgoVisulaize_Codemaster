document.addEventListener("DOMContentLoaded", () => {
	...
	document.getElementById("save-settings").addEventListener("click", () => {
		...
		animations.nodeAnimationTimeout = isNaN(inputs[0].valueAsNumber)
			? 1000
			: inputs[0].valueAsNumber;

		animations.pointerAnimationTimeout = isNaN(inputs[1].valueAsNumber)
			? 800
			: inputs[1].valueAsNumber;

		animations.deleteTimeout = isNaN(inputs[2].valueAsNumber)
			? 1000
			: inputs[2].valueAsNumber;

		succes.innerHTML = '<i class="fas fa-check-circle"></i>' + " Saved!";
